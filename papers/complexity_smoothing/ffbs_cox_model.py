...
"""
Compare different FFBS algorithms on the same example as in the book (Chapter
12). That is, the model is:

X_t|X_{t-1}=x_{t-1} ~ N(mu+phi(x_{t-1}-mu),sigma^2)
Y_t|X_t=x_t ~ Poisson(exp(x_t))

and the additive function is: 

sum_{t=0}^{T-2} \psi_t(X_t, X_{t+1})

see below for a definition of psi_t

Following Dau & Chopin (2022), we compare the following FFBS (forward filtering
backward sampling) algorithms:

* the standard, O(N^2) one 
* FFBS-reject
* hybrid: performs at most N trials in the rejection step; then switch back to
  the standard, expensive method
* FFBS-MCMC: the index of the ancestor is generated by a single step of an
  independent Metropolis kernel, where the starting point is the current
  ancestor.
