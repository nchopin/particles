...
This module gives access to several classical datasets. A dataset is
represented as an instance of class `Dataset` (or one of its sub-classes), with
attributes: 
...
The preprocessing step depends on the class (through method ``preprocess``).
The preprocessing step of class `Dataset` does nothing (both attributes
``raw_data`` and ``data`` point to the same object). The preprocessing step of
sub-class `LogReturnsDataset` computes the log-returns of some price data, and
so on. 
...
Here a quick example::
    ...
    help(dataset)  # basic info on dataset (including URL to source)
...
================= ===================== =========================
    Dataset             sub-class           typical use/model
================= ===================== =========================
...
Neuro              ``Dataset``          neuroscience state-space model
...
class Pima(BinaryRegDataset):
    ...
    pred_names = ['pregnant,' 'glucose', 'pressure', 'triceps', 'insulin',
                  'mass', 'pedigree', 'age']
    response_name = 'diabetes'
